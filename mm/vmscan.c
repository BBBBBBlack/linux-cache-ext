...
out:
  for_each_evictable_lru(lru)
  {
    ...
    case SCAN_FRACT:
       ...
       * round-off error.
       */
      scan = mem_cgroup_online(memcg) ? div64_u64(scan * fraction[file], denominator) : DIV64_U64_ROUND_UP(scan * fraction[file], denominator);
      break;
    case SCAN_FILE:
    case SCAN_ANON:
...
static bool should_skip_mm(struct mm_struct* mm, struct lru_gen_mm_walk* walk)
{
  ...
  {
    size += type ? get_mm_counter(mm, MM_FILEPAGES) : get_mm_counter(mm, MM_ANONPAGES) + get_mm_counter(mm, MM_SHMEMPAGES);
  }
...
static bool should_abort_scan(struct lruvec* lruvec, struct scan_control* sc)
{
  ...
  mark = sysctl_numa_balancing_mode & NUMA_BALANCING_MEMORY_TIERING ? WMARK_PROMO : WMARK_HIGH;
...
static int shrink_one(struct lruvec* lruvec, struct scan_control* sc)
{
  ...
  return lru_gen_memcg_seg(lruvec) != MEMCG_LRU_TAIL ? MEMCG_LRU_TAIL : MEMCG_LRU_YOUNG;
}
